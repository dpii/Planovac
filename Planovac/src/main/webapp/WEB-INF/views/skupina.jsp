...
<table>
	...
	<c:forEach var="udalost" items="${seznamUdalosti}">
		<tr>
			<td><spring:url value="/udalost/${udalost.idUdalosti}"
					context="planovac" var="udalostUrl" htmlEscape="true">
				</spring:url> <a href="${udalostUrl}">${udalost.nazev}</a></td>
			<td>${udalost.popis}</td>
...
<div>
	<c:if test="${opravneni}">
		<a
			href="<spring:url value="/skupiny/${skupina.idSkupiny}/upravit" htmlEscape="true" />">Upravit
			skupinu</a>
		<br />
	</c:if>
	<c:if test="${neniVeSkupine==1}">
		<a
			href="<spring:url value="/skupiny/${skupina.idSkupiny}/pridatSe" htmlEscape="true" />">Přidat
			se ke skupině</a>
		<br />
	</c:if>
	<c:if test="${neniVeSkupine==2}">
		<a
			href="<spring:url value="/skupiny/${skupina.idSkupiny}/odebratSe" htmlEscape="true" />">Odebrat
			se od skupiny</a>
		<br />
	</c:if>
